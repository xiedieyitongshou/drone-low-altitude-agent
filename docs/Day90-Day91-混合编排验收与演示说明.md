...
- 单一自然语言入口支持查询、评估、推荐、比选、追问、修改和知识查询
- AgentState 能表达多轮任务状态，而不是每轮重新开始
- Business Route 把业务目标映射为工具路径
- Context Manager 控制用户输入、session、profile、默认值的优先级
- RAG 调用策略明确，可以调用，也可以跳过
- E2E 测试验证 Agent 行为不再像固定 workflow
